...
DYN_REQ = ("dm_category", "dm_moder_role", "unimice_guild")
...
class DMMessenger(commands.Cog):
    ...
    async def _create_dm_channel(self, user: User) -> int:
        ...
        # get moder role
        dm_moder_role = guild.get_role(self.bot.props["dynamic_config/dm_moder_role"])
        overwrites = {
            dm_moder_role: PermissionOverwrite(view_channel=True),  # добавляем роль модератора в доступ
    ...
    async def _send_dm_msg_handler(self, msg: Message):
        ...
        await channel.edit(name=self.bot.props["dmm/dm_ch_name_new"])
    ...
    async def _send_category_msg_handler(self, msg: Message):
        ...
        await msg.channel.edit(name=self.bot.props["dmm/dm_ch_name"])
    ...
    async def open_dm(self, inter: ApplicationCommandInteraction, user: User):
        ...
        if inter.user.get_role(self.bot.props["dynamic_config/dm_moder_role"]) is None:
